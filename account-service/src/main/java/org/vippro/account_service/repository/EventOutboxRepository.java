...
public interface EventOutboxRepository extends JpaRepository<EventOutbox, UUID> {
    ...
    @Query("""
            ...
            SET e.status = org.vippro.account_service.enums.EventOutboxStatus.NEW,
                e.processingStartedAt = NULL,
                e.nextRetryAt = :now,
                e.lastError = :reason
            WHERE e.status = org.vippro.account_service.enums.EventOutboxStatus.PROCESSING
