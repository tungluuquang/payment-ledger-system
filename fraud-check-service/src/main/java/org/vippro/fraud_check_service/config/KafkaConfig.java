...
public class KafkaConfig {
    ...
    @Bean
    public DefaultErrorHandler fraudCommandErrorHandler(
            @Qualifier("deadLetterKafkaTemplate")
            KafkaTemplate<String, String> deadLetterKafkaTemplate,
            @Value("${fraud.kafka.retry.interval-ms:2000}")
            long retryIntervalMs,
            @Value("${fraud.kafka.retry.max-attempts:3}")
            long maxAttempts
    ) {
    ...
    fraudCommandKafkaListenerContainerFactory(
            ConsumerFactory<String, String> consumerFactory,
            DefaultErrorHandler fraudCommandErrorHandler,
        ...
        factory.setCommonErrorHandler(fraudCommandErrorHandler);
        factory.setAutoStartup(autoStartup);
        return factory;
    }

    @Bean
    public NewTopic fraudCommandsTopic() {
    ...
    @Bean
    public NewTopic fraudCommandsDeadLetterTopic() {
        return TopicBuilder.name("fraud-check-commands.DLT")
                .partitions(3)
                .replicas(1)
                .build();
    }
