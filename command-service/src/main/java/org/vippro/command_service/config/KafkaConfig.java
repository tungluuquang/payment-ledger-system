...
public class KafkaConfig {
    ...
    @Bean
    public DefaultErrorHandler paymentCommandErrorHandler(
            @Qualifier("deadLetterKafkaTemplate")
            KafkaTemplate<String, String> deadLetterKafkaTemplate,
            @Value("${command.kafka.retry.interval-ms:2000}")
            long retryIntervalMs,
            @Value("${command.kafka.retry.max-attempts:3}")
            long maxAttempts
    ) {
    ...
    @Bean
    public NewTopic paymentCommandsDeadLetterTopic() {
        return TopicBuilder.name("payment-commands.DLT")
                .partitions(3)
                .replicas(1)
                .build();
    }
